...
export default function Services() {
  ...
  // WhatsApp logic for specific interest
  const handleInterest = (serviceTitle) => {
    const msg = encodeURIComponent(`Hello Nishanth, I am interested in the ${serviceTitle} service at Dazzling Villa.`);
  ...
  return (
    <section className="bg-[#0A0F0A] py-32 overflow-hidden relative">
      ...
      {/* DYNAMIC AMBIENT GLOW - Changes color slightly when items are hovered */}
          ...
          {[...row1, ...row1].map((item, index) => (
            <div
              ...
              className="group/card flex items-center gap-8 px-12 py-8 bg-white/[0.01] border border-white/5 backdrop-blur-md 
                         hover:bg-white/[0.04] hover:border-[#C9A24D]/40 transition-all duration-700 rounded-sm relative overflow-hidden"
            >
      ...
      <div className="mt-32 flex flex-col items-center justify-center px-6 relative z-10">
        {!isRevealed ? (
          <div className="text-center group cursor-pointer" onClick={() => setIsRevealed(true)}>
            ...
            <button className="px-12 py-5 border border-white/10 text-white text-[10px] tracking-[0.5em] uppercase hover:border-[#C9A24D] transition-all duration-700 relative overflow-hidden group">
              <span className="relative z-10 group-hover:text-[#C9A24D]">Inquire for Custom Experience</span>
              <div className="absolute inset-0 translate-y-full group-hover:translate-y-0 bg-white/[0.02] transition-transform duration-500"></div>
            </button>
          </div>
        ) : (
          <div className="animate-[reveal_1s_cubic-bezier(0.16,1,0.3,1)] w-full max-w-md">
            <div className="relative p-10 bg-[#0F150F] border border-[#C9A24D]/30 backdrop-blur-3xl text-center shadow-[0_30px_100px_rgba(0,0,0,0.8)]">
              ...
              <div className="flex flex-col gap-5">
                <a href="tel:+917904796306" className="group flex items-center justify-between px-6 py-4 border border-white/5 hover:border-[#C9A24D]/20 text-white/70 transition-all">
                  ...
                  <div className="flex items-center gap-3">
                     ...
                     <Phone size={14} className="text-[#C9A24D]" />
                  </div>
                </a>
                
                <a href="https://wa.me/917904796306" target="_blank" className="flex items-center justify-center gap-4 py-4 bg-[#C9A24D] text-black text-[10px] font-bold uppercase tracking-[0.4em] hover:bg-white transition-all">
